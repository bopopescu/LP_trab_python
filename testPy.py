...
class QueueProvider(threading.Thread):
	def __init__(self, xmlFile):
		threading.Thread.__init__(self)
		self.xmlFile = xmlFile
	def run(self):
		root = ET.parse(self.xmlFile).getroot()
		mainData = filter(isDataOfInterest, root[0].attrib.items())
		mainData.append(('NUMERO-IDENTIFICADOR', root.attrib['NUMERO-IDENTIFICADOR']))
#		print mainData
		MessageQueue.stack(mainData, root[0].tag)
...
try:
	qConsumer.start()
	for xmlFile in listdir('xmlFiles'):
		QueueProvider('xmlFiles/'+xmlFile).start()
except:
